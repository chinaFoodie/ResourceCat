 ...
 * @date 2016-6-16 19:16:40
 */
public class AwaitApprovalFragment extends BaseFragment implements OnItemClickListener {
    ...
    private int HTTP_QUERY_AWAIT_APPROVAL = 169;
    private int HTTP_GET_APPROVAL_DETAILS = 170;
    private List<ApprovalModel.ApprovalInfo> lsitAi = new ArrayList<>();
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.arg1 == HTTP_QUERY_AWAIT_APPROVAL) {
                if (msg.what == Integer.parseInt(AppConfig.SUCCESS)) {
                    ApprovalModel am = (ApprovalModel) msg.obj;
                    if (am != null) {
                        lsitAi = am.getData().getResult();
                        awaitAdapter = new AwaitApprovalAdapter(getActivity(), lsitAi);
                        recyclerViewAwait.setAdapter(awaitAdapter);
                        awaitAdapter.setOnItemClickListener(AwaitApprovalFragment.this);
                    }
                } else {
                    Toastor.showToast(getActivity(), msg.obj.toString());
                }
            } else if (HTTP_GET_APPROVAL_DETAILS == msg.arg1) {
                if (msg.what == Integer.parseInt(AppConfig.SUCCESS)) {
                    ...
                    if (adm != null) {
                        ...
                        intent.putExtra("approval_come_from", "await_me_approval");
                        intent.putExtra("approval_id", lsitAi.get(index).getId());
    ...
    private void init() {
        ...
        httpHelper.postStringBack(HTTP_QUERY_AWAIT_APPROVAL, AppConfig.QUERY_AWAIT_APPROVAL, await(), handler, ApprovalModel.class);
    }

    private HashMap<String, String> await() {
        HashMap<String, String> await = new HashMap<String, String>();
        await.put("token", TelephoneUtil.getIMEI(getActivity()));
        await.put("flag", "0");
        await.put("pageNo", "1");
        await.put("pageSize", "1000");
        return await;
    }
    ...
    public void onItemClick(View view, int position) {
        index = position;
        httpHelper.postStringBack(HTTP_GET_APPROVAL_DETAILS, AppConfig.GET_APPROVAL_DETAILS, details(lsitAi.get(position).getId()), handler, ApprovalDetailsModel.class);
    }
    ...
    class AwaitApprovalAdapter extends RecyclerView.Adapter {
        ...
        public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
            ...
            ((AwaitViewHolder) holder).tvApprovalState.setText("待审批");//
